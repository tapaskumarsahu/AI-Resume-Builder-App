...
export default function Dashboard() {
  ...
  const { token} = useSelector(state => state.auth) //set {user,}
      ...
      <div className='max-w-7xl mx-auto px-4 py-8'>
        ...
        <div className='flex gap-4'>
          ...
          <button onClick={()=>setShowUploadResume(true)} className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 text-slate-600 border border-dashed border-slate-300 group hover:border-purple-500 hover:shadow-lg transition-all duration-300 cursor-pointer'>
            <UploadCloudIcon className='size-11 transition-all duration-300 p-2.5 bg-gradient-to-br from-purple-300 to-purple-500 rounded-full text-white'/>
          ...
          {showCreateResume && (
            <form onSubmit={createResume} onClick={() => setShowCreateResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-50 flex items-center justify-center'>
              <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                ...
                <input onChange={(e)=>setTitle(e.target.value)} value={title} type="text" placeholder="Enter resume title" className="px-4 py-2 w-full mb-4 focus:border-indigo-600 ring-indigo-600" required/>
                <button className='w-full py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 transition-colors'>Create Resume</button>
                <XIcon onClick={() => {setShowCreateResume(false); setTitle('')}}  className='size-6 p-1.5 rounded-full text-slate-400 hover:text-slate-600 transition-colors absolute top-4 right-4 cursor-pointer' />
              </div>
            </form>
          )}
          ...
          {showUploadResume && (
            <form onSubmit={uploadResume} onClick={() => setShowUploadResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-50 flex items-center justify-center'>
              <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                ...
                <input onChange={(e)=>setTitle(e.target.value)} value={title} type="text" accept=".pdf,.doc,.docx" className="px-4 py-2 w-full mb-4 focus:border-indigo-600 ring-indigo-600" required placeholder="Enter resume title"/>
                <div>
                  <label htmlFor="resume-input" className='block text-sm text-slate-700'>Select resume file
                    <div className='flex flex-col items-center justify-center gap-2 border group text-slate-400 border-slate-400 border-dashed rounded-md p-4 py-10 my-4 hover:border-indigo-500 hover:text-indigo-700 transition-colors cursor-pointer'>
                      {resume ? (
                  ...
                  <input type="file" id="resume-input" onChange={(e)=>setResume(e.target.files[0])} hidden />
                </div>
                ...
                <XIcon onClick={() => {setShowUploadResume(false); setResume('')}}  className='size-6 p-1.5 rounded-full text-slate-400 hover:text-slate-600 transition-colors absolute top-4 right-4 cursor-pointer' />
              </div>
            </form>
          )}
          ...
          {editResumeId && (
            <form onSubmit={editTitle} onClick={() => setEditResumeId('')} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-50 flex items-center justify-center'>
              <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                ...
                <XIcon onClick={() => {setEditResumeId(''); setTitle('')}}  className='size-6 p-1.5 rounded-full text-slate-400 hover:text-slate-600 transition-colors absolute top-4 right-4 cursor-pointer' />
              </div>
            </form>
          )}
